Function Log {
    ...
    # Write to log file
    Add-Content -Path "$logDir\script.log" -Value $logMessage
}
...
Function CheckServices {
    ...
    $servicePatterns = @('ITSPlatform*', 'SAAZ*')
    $excludedServices = @('SAAZRemoteSupport')
    $foundServices = @()
    $servicesToStart = @()
    ...
    foreach ($pattern in $servicePatterns) {
        ...
        if ($services -ne $null) {
            foreach ($service in $services) {
                ...
                if ($service.Name -notin $excludedServices -and $service.Status -ne 'Running') {
                    Log "Service $($service.Name) is stopped and will be attempted to start."
    ...
    if ($foundServices.Count -eq 0) {
        Log "No services matching the patterns were found. Installing RMM."
        InstallRMM -Token $Token
        return
    }

    if ($servicesToStart.Count -gt 0) {
        $startResult = StartServices -ServiceNames $servicesToStart.Name -Token $Token
        if ($startResult -eq $false) {
            Log "Some services could not be started. Running uninstaller and then installer." -Level "ERROR"
            UninstallRMM
            InstallRMM -Token $Token
        } else {
            Log "All stopped services have been started successfully."
        }
    } else {
        Log "All services are running or excluded. No action needed."
    }
}
...
Function StartServices {
    ...
    foreach ($serviceName in $ServiceNames) {
        try {
            Log "Attempting to start service $serviceName."
            $service = Get-Service -Name $serviceName -ErrorAction Stop
            $service | Start-Service -ErrorAction Stop
            Log "Service $serviceName started successfully."
        } catch {
            Log "Failed to start service $serviceName. Error: $_" -Level "ERROR"
